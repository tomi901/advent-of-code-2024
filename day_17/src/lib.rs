...
pub fn execute_all_instructions(input: &str) -> Vec<TinyByte> {
    ...
    let mut computer = Computer::from_str(computer_s).unwrap();
...
impl Computer {
    ...
    pub fn execute(&mut self, instructions: &[TinyByte]) -> Vec<TinyByte> {
...
mod tests {
    ...
    fn example_2() {
        ...
        let output = computer.execute(&[5, 0, 1, 5, 4]);
    ...
    fn example_3() {
        ...
        let output = computer.execute(&[0, 1, 5, 4, 3, 0]);
